...
public class RaftLog {
    ...
    // @param index of log entry immediately before entries to be deleted
    public int delete(int prevIndex) {
        // create an copy of log
        ...
        Path tmpLogPath;
        try {
            tmpLogPath = FileSystems.getDefault().getPath(mLogPath.toAbsolutePath().toString() + ".tmp");
            OutputStream out = Files.newOutputStream(tmpLogPath, 
                    StandardOpenOption.CREATE,
    ...
    public int insert (Entry[] entries, int prevIndex, int prevTerm) {
        if (entries == null) {
            ...
            System.out.println ("RaftLog: index and term mismatch, could not insert new log entries.");
            return -1;
        }
